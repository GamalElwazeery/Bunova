...
Bunova native source markers remain authoritative while the projection translates them into ExecutionKit lifecycle states: `[ ]` → `READY`, `[~]` → `IMPLEMENTED`, `[!]`/`[B]` → `BLOCKED`, `[R]` → `REVIEW_REQUIRED`, `[x]` → `ACCEPTED`. `[ ]` still means not implemented in canonical Markdown; projecting it as `READY` makes the earliest dependency-satisfied open task executable by the pinned runtime. The projection adds deterministic dependency/routing metadata without owning mutable state.
