...
- [x] `P01-012` Establish versioned API conventions, error envelope, pagination/filtering, idempotency headers and correlation IDs. **Evidence:** Implemented versioned API framework under `/api/v1`: `CorrelationIdMiddleware` propagating `X-Correlation-ID` and binding to logging context; `ApiErrorResponse` standard error envelope (`code`, `message`, `details`, `correlation_id`, `timestamp`) with automatic exception mapping; `PaginationParams` and `QueryFilter` supporting offset and cursor pagination; `IdempotencyRecord`, `IdempotencyService`, and `IdempotencyMiddleware` enforcing atomic lock acquisition, replay caching with `X-Idempotent-Replay: true`, and payload mismatch / concurrent conflict rejection per `IDEMPOTENCY_AND_REPLAY_CONTRACT.md`; verified via `ApiConventionsAndIdempotencyTest` (12 tests, 90 assertions; full backend suite 83 passed, 557 assertions) passing on host.
