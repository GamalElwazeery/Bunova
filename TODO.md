...
- [x] `P01-011` Establish module boundary conventions, domain command/use-case conventions, transaction/outbox policy and dependency rules. **Evidence:** Documented bounded context rules in `docs/01-architecture/MODULE_BOUNDARY_CONVENTIONS.md`; created transactional outbox schema (`outbox_events`) and `OutboxEvent` model; built `DomainCommand`, `CommandHandler`, `CommandBus`, and `OutboxService` providing atomic transaction-bound event recording and dispatching; verified via `ModuleBoundaryAndOutboxTest` (4 tests, 23 assertions; full backend suite 71 passed, 467 assertions) passing on host.
