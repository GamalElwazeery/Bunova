...
- [x] `P01-013` Establish audit event infrastructure per `AUDIT_EVENT_CONTRACT.md` and sensitive-action helpers. **Evidence:** Implemented canonical audit trail infrastructure: database migration `audit_events` with composite indices and foreign keys; `AuditEvent` model with strict immutability guards (preventing mutations and deletions) and mandatory action family constants; `AuditService` providing append-only persistence, automatic request actor/device context extraction, before/after diffing, and automated secret redaction; `RecordsAuditEvents` trait for domain integration; integrated audit logging into `DeviceRegistryService` for device enrollment, suspension, reactivation, revocation, and credential rotation; verified via `AuditEventInfrastructureTest` (7 tests, 30 assertions; full backend suite 90 passed, 587 assertions) passing on host.
