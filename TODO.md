...
- [x] `P01-002` Establish environment/config strategy for local/test/staging/production with secret-safe templates and validation. **Evidence:** Environment strategy defined across local/testing/staging/production in `docs/04-delivery/ENVIRONMENT_CONFIGURATION.md`; secret-safe templates established in `backend/.env.example` and `backend/.env.testing`; validation engine implemented in `App\Support\Config\EnvironmentValidator` and exposed via `php artisan env:validate`; automated tests in `EnvironmentConfigTest` (7 tests, 19 assertions) pass on host.
- [x] `P01-003` Bootstrap PostgreSQL/Redis/Reverb/object-storage local dependencies and health checks; document reproducible startup. **Evidence:** Local dependency compose stack defined in `tooling/docker/docker-compose.yml` (PostgreSQL 16, Redis 7, MinIO S3 with `bunova-media` bucket auto-init, Reverb); startup/shutdown scripts implemented in `tooling/scripts/`; health endpoints (`/up`, `/api/v1/health`) and `php artisan health:check` command established; verified via `HealthCheckTest` (3 tests, 12 assertions) passing on host.
- [x] `P01-004` Implement organization/brand/branch tenancy primitives with explicit organization/branch keys and global IDs. **Evidence:** Tenancy primitives implemented: `Organization`, `Brand`, and `Branch` models in `app/Domain/Identity/Models/` with global UUIDv7 keys, explicit foreign keys, and soft deletes; request-scoped `TenantContext` and Eloquent traits `BelongsToOrganization` / `BelongsToBranch` with global scopes; migrations executed; verified via `TenancyTest` (6 tests, 29 assertions) passing on host.
- [x] `P01-005` Implement user/staff identity separation, authentication foundation and session security suitable for web and registered devices. **Evidence:** Identity separation implemented: `User` (web/cloud accounts with `platform_operator` vs `tenant_user` separation) and `StaffIdentity` (operational personnel with branch scoping, multi-branch assignment, and PIN hashing); `AuthenticationService` and `StaffAuthController` (`POST /api/v1/pos/staff/authenticate`) established; verified via `IdentityAuthTest` (6 tests, 20 assertions) passing on host.
- [x] `P01-006` Implement roles/permissions/policies foundation with branch scoping and negative authorization tests. **Evidence:** Role and permission schema implemented (`roles`, `permissions`, `role_permissions`, `staff_role_assignments`); established `Permission`, `Role`, and `StaffRoleAssignment` models and `PermissionCatalog` defining domain permissions and role templates; implemented `AuthorizationService` and domain policies (`BillPolicy`, `OrderPolicy`) enforcing branch scoping, least-privilege, and platform privilege isolation; verified via `AuthorizationPolicyTest` (8 tests, 37 assertions; full backend suite 34 passed, 155 assertions) passing on host.
- [x] `P01-007` Implement device/register registry, registration/revocation, capability/mode metadata and secure device credential strategy. **Evidence:** Registered device schema implemented (`registered_devices`); established `RegisteredDevice` model with type/status/capability constants; created `DeviceRegistryService`, `AuthenticateDevice` middleware, and `DeviceController` endpoints (`enroll`, `me`, `heartbeat`, `revoke`, `rotate-credentials`) using secure SHA-256 hashed `bnd_` device tokens; verified via `DeviceRegistryTest` (8 tests, 49 assertions; full backend suite 42 passed, 204 assertions) passing on host.
- [x] `P01-008` Implement capability registry/dependency validation and organization/branch configuration. **Contract:** `CAPABILITY_CONFIGURATION_CONTRACT.md`. **Evidence:** Capability registry and management engine implemented (`CapabilityDefinition`, `CapabilityRegistry`, `CapabilityManager`); database schema `capability_configurations` created; supports atomic dependency activation, `needs_configuration` state, active dependent protection, branch overrides, and historical preservation on disable; verified via `CapabilitySystemTest` (10 tests, 41 assertions; full backend suite 52 passed, 245 assertions) passing on host.
- [x] `P01-009` Implement onboarding preset definitions without mode-specific forks; add dependency/disable-history tests. **Evidence:** Implemented canonical preset catalog (`PresetDefinition`, `PresetRegistry`, `PresetService`) defining 8 archetypes (`coffee_cart`, `takeaway_kiosk`, `coffee_shop`, `traditional_cafe`, `cafe_and_restaurant`, `gaming_cafe`, `coworking_cafe`, `multi_branch_chain`) purely as configuration overlays without architecture forks; verified non-destructive switching and history preservation via `OnboardingPresetTest` (6 tests, 131 assertions; full backend suite 58 passed, 376 assertions) passing on host.
- [x] `P01-010` Establish canonical ID, money, currency, timezone/business-day, locale and audit value primitives. **Evidence:** Implemented domain value primitives (`CanonicalId`, `Currency`, `Money`, `BusinessDay`, `Locale`, `AuditContext`) in `App\Domain\Shared\ValueObjects\`; eliminates binary floating-point arithmetic for money, guarantees exact ratio allocation without penny-drop loss, handles operational midnight crossover with timezone-aware business days, provides Arabic RTL/Eastern numeral formatting, and enforces automated secret redaction per `AUDIT_EVENT_CONTRACT.md`; verified via `ValueObjectsTest` (9 tests, 68 assertions; full backend suite 67 passed, 444 assertions) passing on host.
- [x] `P01-014` Establish queue/outbox/inbox infrastructure with idempotent job/event consumption. **Evidence:** Implemented resilient asynchronous pipeline: `inbox_messages` database schema with strict unique constraint `[consumer_name, message_id]`; `InboxMessage` model and `InboxService` executing consumer actions transactionally and preventing duplicate side-effects from at-least-once delivery per `IDEMPOTENCY_AND_REPLAY_CONTRACT.md`; `DomainJob` base queued job restoring tenant context (`organization_id`, `branch_id`) and correlation IDs; `ProcessOutboxJob` and `ProcessOutboxCommand` (`php artisan outbox:process`) dispatching pending outbox events across organizations; `PruneInboxCommand` (`php artisan inbox:prune`) maintaining table hygiene; verified via `QueueOutboxInboxTest` (7 tests, 28 assertions; full backend suite 97 passed, 615 assertions) passing on host.
